      ...
      let anim = gsap.to(span[videoId], {
        ...
        onComplete: () => {
          if (isPlaying) {
            ...
            const animUpdate = () => {
  // Check if videoRef.current and the specific video element exist
  if (videoRef.current && videoRef.current[videoId]) {
    const currentVideo = videoRef.current[videoId]; // The specific video based on videoId
    
    // Check if the video's currentTime and the corresponding video's duration are valid
    if (currentVideo.currentTime && highlightsSlides[videoId].videoDuration) {
      anim.progress(currentVideo.currentTime / highlightsSlides[videoId].videoDuration);
    }
  }
};
